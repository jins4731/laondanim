...
public class TripDao {
	...
	public ArrayList<Trip> selectTripPage(Connection conn, int cPage, int perPage, String lo, String category, String keyword, String recent, String like){
		...
		if(keyword.equals("null")) {
			...
			Pattern pattern = Pattern.compile("LIKE");//찾을 문자
			   Matcher matcher = pattern.matcher(sql);//대상 문자열
			   int count = 0;
			   while (matcher.find()) {
			      count++;
			   }
			   ...
			   System.out.println("indexs.length : " + indexs.length);
			   int targetIndex = indexs[1]; //2번째 Like
			   sql = sql.substring(0, targetIndex) + "!=" + sql.substring(targetIndex+4, sql.length()); //바꿀 문자열
